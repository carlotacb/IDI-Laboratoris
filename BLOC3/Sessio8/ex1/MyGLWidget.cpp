...
void MyGLWidget::createBuffers ()
{
  ...
  patr.load("/home2/users/alumnes/1183742/dades/linux/IDI/LAB/models/Patricio.obj");
  ...
  glm::vec3 diff(0.8,0,0.8); //glm::vec3 diff(0.8,0,0.8);
  glm::vec3 spec(1,1,1);
  float shin = 100; // defineix quant brillant es el material.
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
	...
	case Qt::Key_K: { // canvia focus en X
		posFocus[0] -= 0.5;
		glUniform3fv (posicioFLoc, 1, &posFocus[0]);
		break;
	}
	...
	case Qt::Key_U: { // canvia focus en Y
		posFocus[1] += 0.5;
		glUniform3fv (posicioFLoc, 1, &posFocus[1]);
		break;
	}
	case Qt::Key_J: {
		posFocus[1] -= 0.5;
	...
	case Qt::Key_H: { // canvia focus en Z
		posFocus[2] += 0.5;
		glUniform3fv (posicioFLoc, 1, &posFocus[2]);
		break;
	}
	case Qt::Key_Y: {
		posFocus[2] -= 0.5;
	...
	case Qt::Key_E: {
		colFocus[0] = (float)(qrand()%10)/10;
		colFocus[1] = (float)(qrand()%10)/10;
		colFocus[2] = (float)(qrand()%10)/10;
		glUniform3fv (colorFLoc, 1, &colFocus[0]);
		break;
	}
	case Qt::Key_R: {
	...
	case Qt::Key_T: { //SCM --> modelo
        //tipusFocus = 1;
        posFocus = glm::vec3(1.0, 1.0, 1.0);
        glm::vec4 posFocusAux = View * TG * glm::vec4(posFocus, 1.0);
        //glUniform1i (tipusFocusLoc, tipusFocus);
        glUniform3f (posicioFLoc, posFocusAux.x, posFocusAux.y, posFocusAux.z);
		break;
	}
	
    case Qt::Key_G: { // SCA --> escena
        //tipusFocus = 2;
        posFocus = glm::vec3(1.0, 1.0, 1.0);
        glm::vec4 posFocusAux = View * glm::vec4(posFocus, 1.0);
        //glUniform1i (tipusFocusLoc, tipusFocus);
        glUniform3f (posicioFLoc, posFocusAux.x, posFocusAux.y, posFocusAux.z);
		break;
	}
	
	case Qt::Key_B: { //SCO --> observador
        //tipusFocus = 3;
		posFocus = glm::vec3(0.0, 0.0, 0.0);
        glm::vec4 posFocusAux = glm::vec4(posFocus, 1.0);
        //glUniform1i (tipusFocusLoc, tipusFocus);
        glUniform3f (posicioFLoc, posFocusAux.x, posFocusAux.y, posFocusAux.z);
		break;
    }
