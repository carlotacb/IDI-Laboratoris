...
void MyGLWidget::initializeGL ()
{
  ...
  vermell = 0.8;
  canviaRed(vermell*100);
  verd = 0;
  canviaGreen(verd*100);
  blau = 0.8;
  canviaBlue(blau*100);
...
void MyGLWidget::decideixcamera () {
    if (camera) {
        inicamerainicial();
    }
    else {
...
void MyGLWidget::inicamerainicial () {
    fov = float(M_PI/3.0);
    fovi = fov;
    ra = 1.0f;
    znear = radiEsc;
...
void MyGLWidget::createBuffers ()
{
  ...
  glm::vec3 amb(vermell,verd,blau);
  glm::vec3 diff(vermell,verd,blau);
  glm::vec3 spec(1,1,1);
  float shin = 100;
...
void MyGLWidget::modelTransformPatricio2() {
    ...
    float angle = (((float)M_PI*2.0f) - ((float)M_PI/2.0f));

    glm::mat4 TG(1.f);
    TG = glm::translate(TG, glm::vec3(2, -1, 1));
    TG = glm::rotate(TG, angle, glm::vec3(0,1,0));
    TG = glm::scale(TG, glm::vec3(escala2));
    TG = glm::translate(TG, -centreBPatr);
...
void MyGLWidget::calculaCapsaModel ()
{
  ...
  escala2 = 2.0/(maxy-miny);

  float dx   = maxx - minx;
  float dy   = maxy - miny;
  float dz   = maxz - minz;
  radiPatrik = sqrt(dx * dx + dy * dy + dz * dz)/2.0;

  centreBPatr[0] = (minx+maxx)/2.0;
  centreBPatr[1] = miny;
  centreBPatr[2] = (minz+maxz)/2.0;
}
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  case Qt::Key_A: {
      tx += 0.5;
      posFocus = glm::vec3(tx, 1.0, 0.0);
      glUniform3fv(posicioFLoc, 1, &posFocus[0]);
      if (!camera) inicamera2();
      break;
  }
  case Qt::Key_D: {
      tx -= 0.5;
...
void MyGLWidget::componentRed(int valor) {

    makeCurrent();
    vermell = (float)valor/100;
    createBuffers();
    update();
}

void MyGLWidget::componentGreen(int valor) {

    makeCurrent();
    verd = (float)valor/100;
    createBuffers();
    update();

}

void MyGLWidget::componentBlue(int valor) {

    makeCurrent();
    blau = (float)valor/100;
    createBuffers();
    update();
