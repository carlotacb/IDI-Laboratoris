...
class MyGLWidget : public QOpenGLWidget, protected QOpenGLFunctions_3_3_Core 
{
  ...
  private:
    ...
    // uniform locations
    GLuint transLoc, projLoc, viewLoc, posicioFLoc, colorFLoc;
    // attribute locations
    ...
    glm::vec3 centreBPatr;
    glm::vec3 posFocus, colFocus;
    float escala, escala2;
    // radi de l'escena
    float radiEsc, radiPatrik;
    float tx;
    float ra, fov, znear, zfar, fovi;
    ...
    int xClick, yClick;
    float angleY, angleX;
    bool perspectiva, camera;

    float vermell, verd, blau;

public slots:
    void canviaCamera();
    void componentRed(int valor);
    void componentBlue(int valor);
    void componentGreen(int valor);

signals:
    void activaCheck();
    void canviaRed(int valor);
    void canviaBlue(int valor);
    void canviaGreen(int valor);
};
